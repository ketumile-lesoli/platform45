...
export default function FormPanel() {
  ...
  const contentPanel = () => (
    <Box
        sx={{p:10, bgcolor: '#ff9200', width: 'auto', height: '148vh', zIndex: '500',
        display: 'flex',
    ...
    <ThemeProvider theme={theme}>
      <Grid container component="aside">
        ...
        >
          ...
          <Button variant='round'sx={{transform: checked? 'rotate(180deg)': ''}} onClick={handleChange} >
            <Avatar  sx={{ p:1, bgcolor: 'black', transform: useCheckMobileScreen()?'rotate(90deg)':'' }} alt="image" src={arrowImage} />
          </Button>
          </Box>
        </Grid>
        
        <Grid id="formPanel" item xs={12} sm={7} md={7}>
        <Slide direction={useCheckMobileScreen()?'up':'right'} in={checked} container={containerRef.current} height="100vh">
          {contentPanel()}
        </Slide>
          <Box
            sx={{
